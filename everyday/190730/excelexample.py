...
def bar_kpi():
    ...
    for month in set(monthly_df["month"].tolist()):
        for product in set(monthly_df["Service"].tolist()):
            release_no = monthly_df[(monthly_df['month'] == month) & (monthly_df['Service'] == product)][
                "count_column"].tolist()
            #
...
duration_kpi = pd.DataFrame({"sessionid": ["s1", "s2", "s3", "s4", "s5", "s6", "s7"],
                             "request": [1515677949915, 1515677949925, 1515677949935, 1515677949942, 1515677949944,
                                         1515677949945, 1515677949946],
